...
def check_location_validity(board, column):  #$ is_valid_location
    return board[ROW_COUNT - 1][
        column] == 0  # Checks if the top row of the respective column is equal to 0 or not
        ...
        for r in range(ROW_COUNT):
            if board[r][c] == piece and board[r][c + 1] == piece and board[r][
                    c + 2] == piece and board[r][
                        c + 3] == piece:  # draw the main rectangle
                return True
...
def is_terminal_state(board):
    return win(board, PLAYER_PIECE) or win(board, AI_PIECE) or len(
...
def negamax(board, depth, alpha, beta, color, piece):
    ...
    for col in valid_locations:
        ...
        new_value = -negamax(b_copy, depth - 1, -beta, -alpha, -color,
                             not_piece(piece))[1]  # without_max
        if (new_value > value):
            value = new_value
            column = col
...
def heuristic(matris):
    ...
    def win_modes(matris, piece):
        ...
        for c in range(COLUMN_COUNT - 3):
            for r in range(ROW_COUNT):
                if (matris[r][c]
                        in check) and (matris[r][c + 1] in check) and (
                            matris[r][c + 2] in check) and (matris[r][c + 3]
                                                            in check):
                    new_way = new_way + 1
        ...
        for c in range(COLUMN_COUNT):
            for r in range(ROW_COUNT - 3):
                if (matris[r][c]
                        in check) and (matris[r + 1][c] in check) and (
                            matris[r + 2][c] in check) and (matris[r + 3][c]
                                                            in check):
                    new_way = new_way + 1
        ...
        for c in range(COLUMN_COUNT - 3):
            for r in range(ROW_COUNT - 3):
                if (matris[r][c]
                        in check) and (matris[r + 1][c + 1] in check) and (
                            matris[r + 2][c + 2]
                            in check) and (matris[r + 3][c + 3] in check):
                    new_way = new_way + 1
        ...
        for r in range(ROW_COUNT):
            ...
            pygame.draw.rect(screen, GREEN,
                             (c * SQUARESIZE, r * SQUARESIZE + SQUARESIZE,
                              SQUARESIZE, SQUARESIZE))  # (Width,Height,Y,X)
...
RADIUS = int(
    SQUARESIZE / 2 -
    5)  # RADIUS of the circles (-5 so that the circles arent touching)
...
pygame.draw.rect(
    screen, PURPLE, (0, 0, width, SQUARESIZE)
)  # Prevents top row from getting black (There should be a better solution for this!)
# Sound files
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            pos = event.pos[0]
            if turn == PLAYER:
                pygame.draw.circle(screen, MAGENTA, (pos, int(SQUARESIZE / 2)),
                                   RADIUS)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if turn == PLAYER:
                ...
                col = int(
                    math.floor(pos / SQUARESIZE)
                )  # round down the number you get from pos/SQUARESIZE
                ...
                if check_location_validity(board, col):
                    ...
                    if win(board, PLAYER_PIECE):
                        ...
                        screen.blit(
                            label, (5, 0)
                        )  # Blit --> Just updates a certain part of the screen (based on x, y position)
...
# # Ask for Player 2 Input
    if turn == AI and not game_over:
        ...
        col, minimax_score = negamax(board, 4, -math.inf, math.inf, 1,
                                     AI_PIECE)  # Negamax / Minimax
        if check_location_validity(board, col):
            row = get_next_open_row(board, col)
            pygame.time.wait(500) # Add a little bit of delay for better user experience
            drop_sound.play()
